...
class rule_home_sends_reply_to_client_shared(num_clients:Int) extends rule(num_clients){
  ...
  when(io.en_r){
    ...
    when(((io.home_current_command_in === e_req_shared) && (!(io.home_exclusive_granted_in) && (io.channel2_4_in(io.home_current_client_in) === e_empty)))){
      io.home_sharer_list_out(io.home_current_client_in) := true.B
      io.home_current_command_out := e_empty
      io.channel2_4_out(io.home_current_client_in) := e_grant_shared
    }
  }
...
class rule_home_sends_reply_to_client_exclusive(num_clients:Int) extends rule(num_clients){
  ...
  when(io.en_r){
    ...
    when(((io.home_current_command_in === e_req_exclusive) && (forall_flag && (io.channel2_4_in(io.home_current_client_in) === e_empty)))){
      io.home_sharer_list_out(io.home_current_client_in) := true.B
      io.home_current_command_out := e_empty
      io.channel2_4_out(io.home_current_client_in) := e_grant_exclusive
    }
  }
