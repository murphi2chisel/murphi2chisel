...
class Ln_receive_AcquirePerm_4(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).link(b1)(1).opcode===AcquirePerm)&&((io.Tree_in(i).cache.state===Branch)&&((io.Tree_in(i).master_pending=/=Grant)&&((io.Tree_in(i).master_pending=/=GrantData)&&(io.Tree_in(i).master_pending=/=ReleaseAck))))))){
msg.opcode := AcquirePerm
msg.para := BtoT
io.Tree_out(j).link(b2)(2) := msg
io.Tree_out(i).master_pending := Grant
}
}
}
...
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).link(b1)(1).opcode===AcquirePerm)&&((io.Tree_in(i).cache.state===None)&&((io.Tree_in(i).master_pending=/=Grant)&&((io.Tree_in(i).master_pending=/=GrantData)&&(io.Tree_in(i).master_pending=/=ReleaseAck))))))){
msg.opcode := AcquireBlock
msg.para := NtoT
io.Tree_out(j).link(b2)(2) := msg
io.Tree_out(i).master_pending := GrantData
...
class Ln_receive_AcquireBlock_1(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).link(b1)(1).opcode===AcquireBlock)&&((io.Tree_in(i).link(b1)(1).para===NtoB)&&((io.Tree_in(i).cache.state===None)&&((io.Tree_in(i).master_pending=/=Grant)&&((io.Tree_in(i).master_pending=/=GrantData)&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))))){
msg.opcode := AcquireBlock
msg.para := NtoT
io.Tree_out(j).link(b2)(1) := msg
io.Tree_out(i).master_pending := GrantData
}
}
}
...
class Ln_receive_AcquireBlock_4(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).link(b1)(1).opcode===AcquireBlock)&&((io.Tree_in(i).link(b1)(1).para===NtoT)&&((io.Tree_in(i).cache.state===None)&&((io.Tree_in(i).master_pending=/=Grant)&&((io.Tree_in(i).master_pending=/=GrantData)&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))))){
msg.opcode := AcquireBlock
msg.para := NtoT
io.Tree_out(j).link(b2)(1) := msg
io.Tree_out(i).master_pending := GrantData
...
class Ln_receive_AcquireBlock_5(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).link(b1)(1).opcode===AcquireBlock)&&((io.Tree_in(i).link(b1)(1).para===NtoT)&&((io.Tree_in(i).cache.state===Branch)&&((io.Tree_in(i).master_pending=/=Grant)&&((io.Tree_in(i).master_pending=/=GrantData)&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))))){
msg.opcode := AcquirePerm
msg.para := BtoT
io.Tree_out(j).link(b2)(1) := msg
io.Tree_out(i).master_pending := Grant
...
class Ln_receive_ReleaseData(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when((io.Tree_in(i).link(b1)(3).opcode===ReleaseData)){
io.Tree_out(i).link(b1)(3).opcode := Empty
io.Tree_out(i).cache.state := Tip
io.Tree_out(i).directory(b1) := None
msg.opcode := ReleaseAck
...
class Ln_receive_Grant(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(4).opcode===Grant)&&(io.Tree_in(j).link(b)(4).para===toT)))){
io.Tree_out(i).cache.state := Tip
...
class Ln_receive_GrantData_toB(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(4).opcode===GrantData)&&(io.Tree_in(j).link(b)(4).para===toB)))){
io.Tree_out(i).cache.state := Branch
...
class Ln_receive_GrantData_toT(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(4).opcode===GrantData)&&(io.Tree_in(j).link(b)(4).para===toT)))){
io.Tree_out(i).cache.state := Tip
...
class Ln_receive_ReleaseAck(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
when(((io.Tree_in(j).sons(b)===i.U)&&(io.Tree_in(j).link(b)(4).opcode===ReleaseAck))){
io.Tree_out(j).link(b)(4).opcode := Empty
io.Tree_out(i).master_pending := Empty
...
class Ln_receive_Probe_NtoN(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&(((io.Tree_in(j).link(b)(2).opcode===ProbeBlock)||(io.Tree_in(j).link(b)(2).opcode===ProbePerm))&&((io.Tree_in(i).cache.state===None)&&(io.Tree_in(i).master_pending=/=ReleaseAck))))){
io.Tree_out(j).link(b)(2).opcode := Empty
msg.opcode := ProbeAck
msg.para := NtoN
io.Tree_out(j).link(b)(3) := msg
...
class Ln_receive_ProbePerm_2(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(2).opcode===ProbePerm)&&((io.Tree_in(i).cache.state===Branch)&&(List.range(1, allBranchNum).forall(k => bool2boolean((io.Tree_in(i).directory(k)===None))).B&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))){
io.Tree_out(j).link(b)(2).opcode := Empty
msg.opcode := ProbeAck
msg.para := BtoN
io.Tree_out(j).link(b)(2) := msg
io.Tree_out(i).cache.state := None
}
}
}
class Ln_receive_ProbePerm_3(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(2).opcode===ProbePerm)&&((io.Tree_in(i).cache.state===Branch)&&(List.range(1, allBranchNum).exists(k => bool2boolean((io.Tree_in(i).directory(k)===Branch))).B&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))){
for(k <- 1 until (allBranchNum+1)){
...
class Ln_receive_ProbeBlock_1(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(2).opcode===ProbeBlock)&&((io.Tree_in(i).cache.state===Tip)&&(List.range(1, allBranchNum).forall(k => bool2boolean((io.Tree_in(i).directory(k)===None))).B&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))){
io.Tree_out(j).link(b)(2).opcode := Empty
io.Tree_out(i).cache.state := None
msg.para := TtoN
msg.opcode := ProbeAckData
...
class Ln_receive_ProbeBlock_2(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(2).opcode===ProbeBlock)&&((io.Tree_in(i).cache.state===Tip)&&(List.range(1, allBranchNum).forall(k => bool2boolean((io.Tree_in(i).directory(k)===None))).B&&(io.Tree_in(i).master_pending=/=ReleaseAck)))))){
io.Tree_out(j).link(b)(2).opcode := Empty
...
class Ln_receive_ProbeBlock_3(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b)===i.U)&&((io.Tree_in(j).link(b)(2).opcode===ProbeBlock)&&(io.Tree_in(i).cache.state===Trunk)))){
for(k <- 1 until (allBranchNum+1)){
...
class Ln_send_Release_1(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).cache.state===Branch)&&(List.range(1, allBranchNum).forall(b => bool2boolean((io.Tree_in(i).directory(b)===None))).B&&((io.Tree_in(i).master_pending=/=ReleaseAck)&&((io.Tree_in(i).master_pending=/=Grant)&&(io.Tree_in(i).master_pending=/=GrantData))))))){
msg.opcode := Release
msg.para := BtoN
io.Tree_out(j).link(b2)(3) := msg
io.Tree_out(i).master_pending := ReleaseAck
io.Tree_out(i).cache.state := None
}
}
}
class Ln_send_Release_2(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).cache.state===Tip)&&(List.range(1, allBranchNum).forall(b => bool2boolean((io.Tree_in(i).directory(b)===None))).B&&((io.Tree_in(i).master_pending=/=ReleaseAck)&&((io.Tree_in(i).master_pending=/=Grant)&&(io.Tree_in(i).master_pending=/=GrantData))))))){
msg.para := TtoN
msg.opcode := ReleaseData
io.Tree_out(j).link(b2)(3) := msg
io.Tree_out(i).master_pending := ReleaseAck
io.Tree_out(i).cache.state := None
}
...
class Ln_send_Release_3(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,i:Int,j:Int,b1:Int,b2:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
var msg = Wire(new msgType(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int))
when(((io.Tree_in(j).sons(b2)===i.U)&&((io.Tree_in(i).cache.state===Tip)&&(List.range(1, allBranchNum).forall(b => bool2boolean((io.Tree_in(i).directory(b)===None))).B&&((io.Tree_in(i).master_pending=/=ReleaseAck)&&((io.Tree_in(i).master_pending=/=Grant)&&(io.Tree_in(i).master_pending=/=GrantData))))))){
msg.para := TtoN
...
class L3_send_release_1(coreNum:Int,allNodeNum:Int,allBranchNum:Int,ChanNum:Int,b:Int) extends node(coreNum,allNodeNum,allBranchNum,ChanNum){
when(io.en_r){
when(((io.Tree_in(1).cache.state===Branch)&&List.range(1, allBranchNum).forall(k => bool2boolean((io.Tree_in(1).directory(k)===None))).B)){
