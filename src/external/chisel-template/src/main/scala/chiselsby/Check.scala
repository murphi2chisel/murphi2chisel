...
object Check {
  ...
  def sby(mode: String = "prove", engines: String = "smtbmc boolector", files: Array[String], module: String) = {
    s"""[options]
       |mode $mode
       |
       |[engines]
  ...
  def bmc[T <: RawModule](dutGen: () => T) = {
    check(dutGen, "bmc")
  }

  def kInduction[T <: RawModule](dutGen: () => T) = {
    check(dutGen, "prove")
  }

  def pdr[T <: RawModule](dutGen: () => T) = {
    check(dutGen, "abcPdr")
  }


  private def check[T <: RawModule](dutGen: () => T, mode: String) = {
    ...
    val sbyFileContent = sby(modeStr, engines, files, mod)
