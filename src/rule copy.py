...
class simplerule(rule):
    ...
    def __init__(self, name: str, condition: expr, body, enclosures, vars):
        ...
        self.enclosures = enclosures  # enclosing ruleset params (if no ruleset , it should be None)
...
class startstate(rule):
    ...
    def generate_code(self):
        ...
        if self.enclosures is not None:
            # the rule is enclosed by paras
            for x in self.enclosures:
                class_paras.append("%s <- %s" % (x[0], x[1].generate_forindex()))
...
class invariant(rule):
    ...
    def generate_code(self):
        ...
        for i in class_paras:
            res += "for(%s){\n" % i
        res += "assert(%s)" % self.body.generate_reg()
        for i in class_paras:
            res += "}\n"
        return res
...
class program(object):
            ...
            if isinstance(rule, simplerule):
                if rule.enclosures is None:
                    ...
                    for enclosure in rule.enclosures:
                        t = t * (enclosure[1].get_size_of_subrange())
                    res += t
        return res
    ...
    def generate_node(self):
        ...
        res += "})\n"
        for d in varmap.values():
            res += "io.%s_out:=io.%s_in\n" % (d.generate_code(), d.generate_code())
        res += "}\n"
        return res
    ...
    def generate_system(self):
        ...
        # generate io
        res += "val io = IO(new Bundle {\n"
        res += "val en_a = Input(UInt(log2Ceil(%d).W))\n" % self.get_num_of_rule()
        for d in varmap.values():
            res += "%s\n" % d.generate_io_out()
        res += "})\n"
