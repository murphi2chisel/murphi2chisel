...
assert(((Cache_reg(1).State===E)&&(Cache_reg(2).State===E)))
for(i <- 1 until (NODE_NUM+1)){
for(j <- 1 until (NODE_NUM+1)){
assert((!(i.U=/=j.U)||((!(Cache_reg(i).State===E)||(Cache_reg(j).State===I))&&(!(Cache_reg(i).State===S)||((Cache_reg(j).State===I)||(Cache_reg(j).State===S))))))
}
}
for(i <- 1 until (NODE_NUM+1)){
assert((!((!(ExGntd_reg===false.B)||(MemData_reg===AuxData_reg))&&(Cache_reg(i).State=/=I))||(Cache_reg(i).Data===AuxData_reg)))
}
}